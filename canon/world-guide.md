<!-- Published copy of ALDERHOARD's own UNIVERSE.md. Section 5 (the layer
     contract) is replaced with a note; nothing else is changed. -->

# ALDERHOARD — Universe Guide
...
to add a game, a room, or a creature to Alderhoard — or to lift the whole thing
...
ship it, sell it. Credit ALDERHOARD and you are square with us.

---

## 1. What Alderhoard is
...
Alderhoard has no list. To reach a game you walk to the thing it lives in and
...
## 5. — removed from the public copy

The section that stood here is the LAYER CONTRACT: the exact interface a
cabinet uses to plug into the Alderhoard shell — which globals it may read,
what `enter()` and `exitDone()` must do, how the test hooks are shaped.

It is not published, and the reason is worth being straight about rather than
silently omitting it. **That contract is the extraction kit.** Each cabinet is
one self-contained IIFE that talks to the shell through about forty globals, so
the contract is most of what anyone would need to lift a finished game out of
Alderhoard and ship it as their own. Everything else in this guide is the
world, and the world is yours. The plumbing of one particular implementation is
not part of the world.

If you are building your own valley, you do not want our interface anyway —
you want the idea in section 1, which is free: **a door is not a menu.**
